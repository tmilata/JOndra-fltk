...
typedef struct Z80_STATE_ {
        ...
        int             status;
        volatile int    break_requested;
        volatile int    temp_breakpoint_enabled;
        volatile int    temp_breakpoint_hit;
        int             temp_breakpoint_address;

        /* Five persistent debugger execution breakpoint slots. */
        unsigned char   user_breakpoint_enabled[5];
        int             user_breakpoint_address[5];
        volatile int    user_breakpoint_hit;
        volatile int    user_breakpoints_suspended;

        /* Debugger memory read/write breakpoints.
         * The CPU state backup lets a hit return to the beginning of the
         * instruction that attempted the watched memory access. */
        volatile int    memory_breakpoint_hit;       /* 0=none, 1=read, 2=write */
        int             memory_breakpoint_hit_address;
        int             memory_read_breakpoint_enabled;
        int             memory_read_breakpoint_address;
        int             memory_write_breakpoint_enabled;
        int             memory_write_breakpoint_address;
        int             memory_breakpoint_skip_enabled;
        int             memory_breakpoint_skip_pc;
        int             memory_breakpoint_instruction_active;
        unsigned char   memory_breakpoint_backup_registers[14];
        unsigned char   memory_breakpoint_backup_alternates[8];
        int             memory_breakpoint_backup_i;
        int             memory_breakpoint_backup_r;
        int             memory_breakpoint_backup_pc;
        int             memory_breakpoint_backup_iff1;
        int             memory_breakpoint_backup_iff2;
        int             memory_breakpoint_backup_im;
        int             memory_breakpoint_backup_pc_step;
        int             memory_breakpoint_backup_status;
        int             memory_breakpoint_backup_pending_ei;
        bool            memory_breakpoint_backup_halt;
        ...
        int             i, r, pc, iff1, iff2, im, pc_step;
        /* EI blocks maskable interrupts until one following instruction has
         * completed.  Normally the old core kept this state only implicitly
         * by extending the current emulate() slice.  Timeline restore can
         * resume exactly on the boundary after EI, so the state must also be
         * persistent.  0=no delay, 1=one instruction still protected. */
        int             pending_ei;
		bool			bHalt;

        uint8_t  *memory;

        uint8_t (*fetchbyte)(uint16_t);
...
#define Z80_FETCH_OPCODE_BYTE(address, x)                               \
{                                                                       \
        (x) = state->fetchbyte((address) & 0xffff);                     \
}
