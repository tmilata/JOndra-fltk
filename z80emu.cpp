...
void Disassmb(char* strInstr,uint16_t myPC){
	
	strInstr[0]='\0';	
	Disassemble(myPC,strInstr);
	int nStrLen=strlen(strInstr);
	
	if(nStrLen<30){
		for(int i=0;i<15-nStrLen;i++){
			strcat(strInstr," ");
		}
	}
...
/*
 * Return non-zero when one of the five persistent debugger breakpoint slots
 * matches the next instruction address.  The check happens before opcode
 * fetch, so execution stops before the breakpoint instruction is executed.
 */
static int user_debug_breakpoint_at(Z80_STATE *state, int address) {
	int i;
	address &= 0xffff;

	if (state->user_breakpoints_suspended) {
		return 0;
	}

	for (i = 0; i < 5; ++i) {
		if (state->user_breakpoint_enabled[i] &&
			((state->user_breakpoint_address[i] & 0xffff) == address)) {
			state->user_breakpoint_hit = 1;
			return 1;
		}
	}
	return 0;
...
static void memory_debug_backup_instruction(Z80_STATE *state, int pc, int r) {
	memcpy(state->memory_breakpoint_backup_registers, state->registers.byte, 14);
	memcpy(state->memory_breakpoint_backup_alternates, state->alternates.byte, 8);
	state->memory_breakpoint_backup_i = state->i;
	state->memory_breakpoint_backup_r = (state->r & 0x80) | (r & 0x7f);
	state->memory_breakpoint_backup_pc = pc & 0xffff;
	state->memory_breakpoint_backup_iff1 = state->iff1;
	state->memory_breakpoint_backup_iff2 = state->iff2;
	state->memory_breakpoint_backup_im = state->im;
	state->memory_breakpoint_backup_pc_step = pc & 0xffff;
	state->memory_breakpoint_backup_status = state->status;
	state->memory_breakpoint_backup_pending_ei = state->pending_ei;
	state->memory_breakpoint_backup_halt = state->bHalt;
	state->memory_breakpoint_instruction_active = 1;
}

static void memory_debug_restore_instruction(Z80_STATE *state, int *pc, int *r) {
	memcpy(state->registers.byte, state->memory_breakpoint_backup_registers, 14);
	memcpy(state->alternates.byte, state->memory_breakpoint_backup_alternates, 8);
	state->i = state->memory_breakpoint_backup_i;
	state->r = state->memory_breakpoint_backup_r;
	state->pc = state->memory_breakpoint_backup_pc;
	state->iff1 = state->memory_breakpoint_backup_iff1;
	state->iff2 = state->memory_breakpoint_backup_iff2;
	state->im = state->memory_breakpoint_backup_im;
	state->pc_step = state->memory_breakpoint_backup_pc_step;
	state->status = state->memory_breakpoint_backup_status;
	state->pending_ei = state->memory_breakpoint_backup_pending_ei;
	state->bHalt = state->memory_breakpoint_backup_halt;
	*pc = state->memory_breakpoint_backup_pc & 0xffff;
	*r = state->memory_breakpoint_backup_r & 0x7f;
}
...
void Z80Reset(Z80_STATE *state) {
	...
	state->temp_breakpoint_address = 0;
	/* Persistent user breakpoint slots survive CPU Reset. */
	state->user_breakpoint_hit = 0;
	state->user_breakpoints_suspended = 0;
	/* Memory breakpoint configuration is persistent, only transient state resets. */
	...
	*/
	state->registers.word[Z80_BC] = 0xffff;
	state->registers.word[Z80_DE] = 0xffff;
	state->registers.word[Z80_HL] = 0xffff;
	state->registers.word[Z80_IX] = 0xffff;
	state->registers.word[Z80_IY] = 0xffff;
	state->alternates.word[Z80_AF] = 0xffff;
	state->alternates.word[Z80_BC] = 0xffff;
	state->alternates.word[Z80_DE] = 0xffff;
	state->alternates.word[Z80_HL] = 0xffff;
	
	state->r = state->i = state->pc = state->pc_step = state->iff1 = state->iff2 = 0;
...
int Z80Interrupt(Z80_STATE *state, int data_on_bus) {
	state->status = 0;
	state->bHalt = false;
	if (state->iff1 && state->pending_ei == 0) {
...
*/
		...
		case Z80_INTERRUPT_MODE_1: {
			...
			state->pc = 0x0038;
			state->addtstates(elapsed_cycles + 13);
		...
		case Z80_INTERRUPT_MODE_2: {
			...
			state->addtstates(elapsed_cycles + 19);
...
int Z80Emulate(Z80_STATE *state, int number_cycles) {
	...
	/* Publish the state at the instruction boundary.  MemoryTimeline ignores
	 * an identical duplicate when a new 20 ms emulation slice starts. */
	if (Ondra::machine != NULL && Config::bEnableTimeline) {
		Ondra::machine->timelineInstructionBoundary();
	}
	/* Stop before executing the instruction at the temporary debugger BP. */
	if (state->temp_breakpoint_enabled &&
		((state->pc & 0xffff) == (state->temp_breakpoint_address & 0xffff))) {
		state->temp_breakpoint_enabled = 0;
		state->temp_breakpoint_hit = 1;
		state->pc_step = state->pc & 0xffff;
		state->elapsed_cycles = 0;
		return 0;
	}
	/* Persistent user debugger breakpoints stop at the same instruction boundary. */
	if (user_debug_breakpoint_at(state, state->pc)) {
		state->pc_step = state->pc & 0xffff;
		state->elapsed_cycles = 0;
		return 0;
	}
	if (state->bHalt){
		state->addtstates(number_cycles);
		return number_cycles;
	}

	state->pc_step = state->pc & 0xffff;
	Z80_FETCH_OPCODE_BYTE(state->pc, opcode);
	state->pc++;
...
*/
	...
	int elapsed_cycles, pc, r, i,elapsed_cycles_last;
	void *register_table[16], *dd_register_table[16], *fd_register_table[16];

	elapsed_cycles = elapsed_cycles_last = 0;

	pc = state->pc;
	r = state->r & 0x7f;

	/* The first opcode was already fetched by Z80Emulate().  No CPU-visible
	 * state changed yet, so pc_step is still the start of this instruction. */
	memory_debug_backup_instruction(state, state->pc_step, r);
	...
	for (;;) {
		...
		/*
		Ondra::machine->mem->copyRamToByteArray(Opcodes);

		nCorrLen=pc;
		nOPCodeLen=OpcodeLen(nCorrLen);
		strBajty[0]='\0';
		strBajt[0]='\0';
		
		for(ind=nCorrLen;ind<nCorrLen+nOPCodeLen;ind++){
			sprintf(strBajt,"%2.2X",Opcodes[ind]);
			strcat(strBajty,strBajt);
		}
		
		Disassmb(strInstr,nCorrLen);;
		
		
		for (int i = 0; i < 15 - strlen(strInstr); i++) {
			strcat(strInstr," ");
            }
		
		CDebug::debug("%4.4X : %s\t%s  A=%2.2X F=%2.2X BC=%4.4X DE=%4.4X HL=%4.4X IX=%4.4X IY=%4.4X SP=%4.4X (SP)=%4.4X",pc,strInstr,strBajty,CPU_GetReg8(Ondra::machine->cpu,REG_A),CPU_GetReg8(Ondra::machine->cpu,REG_F),CPU_GetReg16(Ondra::machine->cpu,REG_BC),CPU_GetReg16(Ondra::machine->cpu,REG_DE),CPU_GetReg16(Ondra::machine->cpu,REG_HL),CPU_GetReg16(Ondra::machine->cpu,REG_IX),CPU_GetReg16(Ondra::machine->cpu,REG_IY),CPU_GetReg16(Ondra::machine->cpu,REG_SP),((CPU_GetReg16(Ondra::machine->cpu, REG_SP) == 0xFFFF ? 0x00 : Ondra::machine->mem->readByte(CPU_GetReg16(Ondra::machine->cpu, REG_SP) + 1)) << 8)|Ondra::machine->mem->readByte(CPU_GetReg16(Ondra::machine->cpu, REG_SP)));
		
*/
		/* Address breakpoints are checked at an instruction boundary, before fetch. */
		if (state->temp_breakpoint_enabled &&
			((pc & 0xffff) == (state->temp_breakpoint_address & 0xffff))) {
			state->temp_breakpoint_enabled = 0;
			state->temp_breakpoint_hit = 1;
			goto stop_emulation;
		}
		if (user_debug_breakpoint_at(state, pc)) {
			goto stop_emulation;
		}

		state->pc_step = pc & 0xffff;
		memory_debug_backup_instruction(state, pc, r);
		Z80_FETCH_OPCODE_BYTE(pc, opcode);
		pc++;
		...
		case HALT: {
			...
			if (state->pending_ei > 0) state->pending_ei--;
			goto stop_emulation;
		...
		case DI: {
			...
			/*
			 * Do not extend the caller's cycle budget here.  The old core
			 * forced one extra instruction by increasing number_cycles, but
			 * that accumulates every time DI is encountered and can extend a
			 * 20 ms frame by hundreds of T-states.  IFF is already cleared,
			 * so a following external INT cannot be accepted after DI.
			 */
			break;
		...
		case EI: {
			...
			/*
			 * pending_ei survives CPU_Emulate() call boundaries and blocks
			 * INT for exactly the following instruction.  Extending
			 * number_cycles is therefore both redundant and harmful to the
			 * frame timing/timeline slice boundary.
			 */
			break;
		...
		case CB_PREFIX: {
			...
			if (registers != register_table) {
				...
				Z80_FETCH_OPCODE_BYTE(pc + 1, opcode);
		...
		state->addtstates(elapsed_cycles-elapsed_cycles_last);
		elapsed_cycles_last=elapsed_cycles;
		state->elapsed_cycles = elapsed_cycles;

		/* The core keeps PC and the low seven bits of R in local variables while
		 * running.  Publish them at the completed-instruction boundary so the
		 * timeline sees the same architectural state as the debugger. */
		...
		} else if (Ondra::machine != NULL && Config::bEnableTimeline) {
			/* This instruction is rolled back by the memory-breakpoint path. */
			Ondra::machine->timelineDiscardPendingChanges();
		}
		
		/* Debugger stop: finish current instruction, then publish a consistent CPU state. */
		if (state->break_requested)
			goto stop_emulation;
...
stop_emulation:
	...
	if (state->memory_breakpoint_hit != CPU_MEM_BREAK_NONE &&
		state->memory_breakpoint_instruction_active) {
		memory_debug_restore_instruction(state, &pc, &r);
	}
	state->memory_breakpoint_instruction_active = 0;
