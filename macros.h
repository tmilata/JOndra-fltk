...
/* Memory breakpoints are detected by Ondra::peek8/poke8.  Abort the
 * instruction immediately after the watched access so no later side effect
 * can escape before the CPU state is restored at stop_emulation. */
#define ABORT_ON_MEMORY_BREAKPOINT()                                    \
{                                                                       \
        if (state->memory_breakpoint_hit)                               \
                goto stop_emulation;                                    \
}
